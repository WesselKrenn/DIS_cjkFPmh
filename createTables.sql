...
CREATE UNLOGGED TABLE StudentRegistrationsToDegrees(StudentRegistrationId int, StudentId int, DegreeId int, RegistrationYear int, PRIMARY KEY (StudentRegistrationId), FOREIGN KEY StudentId REFERENCES Students(StudentId), FOREIGN KEY DegreeId REFERENCES Degrees(DegreeId));
...
CREATE UNLOGGED TABLE Courses(CourseId int, CourseName varchar(50), CourseDescription varchar(200),DegreeId int, ETCS int, PRIMARY KEY (CourseId), FOREIGN KEY DegreeId REFERENCES Degrees(DegreeId));
CREATE UNLOGGED TABLE CourseOffers(CourseOfferId int, CourseId int, Year int, Quartile int, PRIMARY KEY (CourseOfferId), FOREIGN KEY CourseId REFERENCES Courses(CourseId));
