...
\timing
--with PassedCourses(CourseName, Grade, Year, Quartile, CourseOfferId) as (SELECT CourseName, Grade, Year, Quartile, co.CourseOfferId FROM Courses c, CourseOffers co, CourseRegistrations cr WHERE c.CourseId = co.CourseId and co.CourseOfferId = cr.CourseOfferID and grade >= 5 and DegreeId = 1 and StudentRegistrationId in (SELECT StudentRegistrationId FROM StudentRegistrationsToDegrees WHERE StudentId = 1) ORDER BY year, quartile, CourseOfferId) SELECT CourseName, grade FROM PassedCourses;

--with PassedCourses(CourseName, Grade,year, QUartile, CourseOfferId) as (Select CourseName, Grade, Year, Quartile, cco.CourseOfferId from courses_courseoffers cco, courseregistrations cr where cco.courseofferid = cr.courseofferid and grade >= 5 and degreeid = 1 and studentregistrationid in (select studentregistrationid from studentregistrationstodegrees where studentid = 1) order by year, quartile, courseofferid) select coursename, grade from passedcourses;
...
-- takes 12s
with CurrentStudent as (SELECT * FROM StudentRegistrationsToDegrees WHERE degreeid = 1099 AND studentid = 2) SELECT c.CourseName, cr.Grade FROM Students s INNER JOIN CurrentStudent cs on (s.studentid = cs.studentid) INNER JOIN CourseRegistrations cr on (cs.studentregistrationid = cr.studentregistrationid) INNER JOIN CourseOffers co on (cr.courseofferid = co.courseofferid) INNER JOIN Courses c on (co.courseid = c.courseid);

-- takes 2s but no result
with CurrentStudent as (SELECT * FROM StudentRegistrationsToDegrees WHERE degreeid = 1099 AND studentid = 2), CurrentCourses as (Select courseid, coursename, degreeid FROM courses where degreeid = 1099) SELECT cc.coursename, cr.grade FROM Currentstudent cs INNER JOIN CourseRegistrations cr on (cr.studentregistrationid = cs.studentregistrationid) INNER JOIN courseoffers co on (cr.courseofferid = co.courseofferid) INNER JOIN currentcourses cc on (co.courseid = cc.courseid);

-- takes 14s
with currentStudent as (SELECT * FROM StudentRegistrationsToDegrees WHERE degreeid = 1099 AND studentid = 2) SELECT c.CourseName, cr.Grade FROM currentstudent cs INNER JOIN CourseRegistrations cr on (cs.studentregistrationid = cr.studentregistrationid) INNER JOIN CourseOffers co on (cr.courseofferid = co.courseofferid) INNER JOIN Courses c on (co.courseid = c.courseid);

-- Takes 0.2s lol (primary key swapped from coid, sr to sr,coid) + added hash index on srtd(sid)
SELECT c.CourseName, creg.Grade FROM StudentRegistrationsToDegrees as sreg, CourseRegistrations as creg, CourseOffers as coffer, Courses as c where sreg.StudentId = %1% and sreg.StudentRegistrationId = creg.StudentRegistrationId AND creg.CourseOfferId = coffer.CourseOfferId AND coffer.CourseId = c.CourseId AND sreg.DegreeId = %2%;
...
-- Q3 
-- Takes 18 seconds good enough I guess
...
-- Q4
-- takes a few (1-6) seconds (is executed 10 times)
